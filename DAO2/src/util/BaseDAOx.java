...
public class BaseDAOx {
    ...
    public int insertGetGeneratedKey(Object ob) {
        ...
        sb.append(")");
        try {
            ps = conn.prepareStatement(sb.toString(), RETURN_GENERATED_KEYS);
            for (int i = 1; i < col.size(); i++) {
                for (Field ff : fi) {
                    if (ff.getName().equals(col.get(i))) {
                        ff.setAccessible(true);
    ...
    public int updateGeneratedKey(Object ob) {
        int id = 0;
        Connection conn = BaseConnection.getConnection();
        PreparedStatement ps = null;
        ResultSet rs = null;
    ...
    /**
     * *
     * 该方法用于 所有查询操作 注意一点 尽量保证 连接的表中数据 除了主外键关联的以外 尽量不要有重复的表列名 参数1：查询出来的数据 放到哪个
     * 实体类中 参数2：查询的sql语句 参数3：所有的条件的值
     *
     * @return
     */
    ...
    public ArrayList getListBySql(Class cl, String sql, Object[] obs) {
        ArrayList ar = new ArrayList();
        Connection conn = BaseConnection.getConnection();
        PreparedStatement ps = null;
        ResultSet rs = null;
...
//        ArrayList<String> arrayList=bd.getAllColumn("animals");
//        for (String str : arrayList) {
//            System.out.println(str);
//        }
        //ArrayList<String> arrayList=bd.getAllColumns("animals");
...
//	an.setAge(12);
//	an.setAnId(1);
//        int id=bd.updateGeneratedKey(an);
//        System.out.println(id);
//        ArrayList<Animals> ar = bd.getListByLike(Animals.class, "name", "泉");
//        for(Animals an: ar){
//            System.out.println(an.getName());
//        }
        String sql = "select* from Animals a ,antype t where a.anid = t.anid and anname=?";
        Object[] ob = {"狗"};
        ArrayList<Animals> ar = bd.getListBySqlX(Animals.class, sql, ob);
        for (Animals an : ar) {
            System.out.println(an.getName());
        }
    }
